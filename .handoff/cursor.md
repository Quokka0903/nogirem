...
Last Updated: 2026-09-16 12:00

## Current Objective
0.3.15 시작 준비 순서와 Vulkan 관리 창 표시 변경을 사용자 환경에서 검증한다.

## Current Status
- 관리 창은 `loadInstalled()`가 캐시·설치·게임 적용 상태를 렌더한 뒤에만 `content-ready`를 보내므로 정적 `확인 전` 셀렉터가 fade 중 노출되지 않는다. 현재 저장본과 최신 캐시는 v3.1.1로 일치하지만 `C:\Nexon\Mabinogi\d3d9_dxvk.dll`이 실제로 없어 메인 `업데이트 필요` 상태는 디스크 판정과 일치한다. 설치 직후 상태 재조회에서 파일이 없거나 무결성·선택 버전·배포가 불일치하면 더 이상 성공 문구를 덮어쓰지 않고 오류를 표시한다. `updateDxvk()`는 확정 runtime 상태와 배포 결과를 로그에 기록하고 primary renderer 이벤트로 전달한다. DXVK 테스트 11개와 Vite 빌드가 통과했다.
